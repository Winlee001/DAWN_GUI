...
from .utils import init_weights, weights_init_kaiming
...
class DBSN_branch(nn.Module):
    def __init__(self, in_ch,inplanes, bs_conv_type, bs_conv_bias, bs_conv_ks, block_num, activate_fun,mask_shape='o'):
        ...
        if self.bs_conv_type != 'Mask3D':
            ...
            self.central_mask_conv = BlindSpotConv(inplanes, inplanes, bs_conv_ks, stride=1, dilation=1, bias=bs_conv_bias, conv_type=bs_conv_type,mask_shape=mask_shape)
            ...
            self.central_mask_conv = BlindSpotConv(inplanes,inplanes,(in_ch,bs_conv_ks,bs_conv_ks), stride=1, dilation=(0,1,1), bias=bs_conv_bias, conv_type=bs_conv_type,padding=(0,p,p),mask_shape=mask_shape)
...
class DBSN_centrblind(nn.Module):
    def __init__(self, in_ch, out_ch, mid_ch, 
                ...
                activate_fun,mask_shape='o'):
        super(DBSN_centrblind,self).__init__()
        #
        if activate_fun == 'Relu':
            # self.relu = nn.ReLU(inplace=True)
            self.relu = partial(nn.ReLU, inplace=True)
        ...
        if blindspot_conv_type == 'MulMask':
            self.br1 = DBSN_branch(in_ch,mid_ch,'MulMask', blindspot_conv_bias, br1_blindspot_conv_ks, br1_block_num, activate_fun,mask_shape)
            self.br2 = DBSN_branch(in_ch,mid_ch,'Mask', blindspot_conv_bias, br2_blindspot_conv_ks, br2_block_num, activate_fun,mask_shape)
        else:
            self.br1 = DBSN_branch(in_ch,mid_ch, blindspot_conv_type, blindspot_conv_bias, br1_blindspot_conv_ks, br1_block_num, activate_fun,mask_shape)
            self.br2 = DBSN_branch(in_ch,mid_ch, blindspot_conv_type, blindspot_conv_bias, br2_blindspot_conv_ks, br2_block_num, activate_fun,mask_shape)
