...
class BiConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        hidden_state = self._init_hidden(Batch_size=input_tensor.size(0), height= H,width= W,cuda=input_tensor.is_cuda)
    ...
    def _init_hidden(self, Batch_size,height,width,cuda):
        init_states = []
        for i in range(self.num_layers):
            if(cuda):
                init_states.append((Variable(torch.zeros(Batch_size, self.hidden_dim[i], height, width).cuda()).cuda(),
                                    Variable(torch.zeros(Batch_size, self.hidden_dim[i], height, width).cuda()).cuda()))
            else:
                init_states.append((Variable(torch.zeros(Batch_size, self.hidden_dim[i], height, width)),
                                    Variable(torch.zeros(Batch_size, self.hidden_dim[i], height, width))))
        return init_states
