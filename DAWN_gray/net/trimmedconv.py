...
class TrimmedConv2d(nn.Conv2d):
    ...
    def get_weight_mask(self,direction=None):
        ...
        weight[:, :, self.kernel_size[0] // 2, self.kernel_size[1] // 2] = 1e-8
    ...
    def forward(self, input):
        #item() is used to extract the value from a one-value tensor
...
class MaskConv2d(nn.Conv2d):
    ...
    def get_weight_mask(self):
        ...
        #weight[:, :, self.kernel_size[0] // 2, self.kernel_size[1] // 2] = 0.1
        # weight[:, :, self.kernel_size[0] // 2, self.kernel_size[1] // 2] = 1e-6
        return torch.tensor(weight.copy(), dtype=torch.float32)
...
class MaskConv3d(nn.Conv3d):
    def __init__(self, *args, **kwargs):
        ...
        self.mask_shape = kwargs.pop('mask_shape', None)

        super(MaskConv3d, self).__init__(*args, **kwargs)
        self.weight_mask = self.get_weight_mask()

    # remove the center position, [1 1 1;1 0 1;1 1 1]
    def get_weight_mask(self):
        weight = np.ones((1, 1, self.kernel_size[0], self.kernel_size[1],self.kernel_size[2]))
        if self.mask_shape=='o':
            #TODO
            weight[:, :, self.kernel_size[0] // 2, self.kernel_size[1] // 2,self.kernel_size[2] // 2] = 0
            #weight[:, :, self.kernel_size[0] // 2, self.kernel_size[1] // 2] = 0.1
            # weight[:, :, self.kernel_size[0] // 2, self.kernel_size[1] // 2] = 1e-6
        elif self.mask_shape=='+':
            print('Using + shape mask for MaskConv3d')
            weight[:, :, self.kernel_size[0] // 2, :,self.kernel_size[2] // 2] = 0
            weight[:, :, self.kernel_size[0] // 2, self.kernel_size[1] // 2,:] = 0
        else:
            raise BaseException("Invalid mask shape!")
        return torch.tensor(weight.copy(), dtype=torch.float32)
    ...
    def forward(self, input):
        if self.weight_mask.type() != self.weight.type():
            with torch.no_grad():
                self.weight_mask = self.weight_mask.type(self.weight.type())
        w=torch.mul(self.weight,self.weight_mask)
        output = F.conv3d(input, w, self.bias, self.stride,
                           self.padding, self.dilation, self.groups)
        return output



class fSzMaskConv2d(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        super(fSzMaskConv2d, self).__init__(*args, **kwargs)
        self.weight_mask = self.get_weight_mask()
    ...
    def get_weight_mask(self):
        weight = np.ones((1, 1, self.kernel_size[0], self.kernel_size[1]))
        #TODO
        weight[:, :, self.kernel_size[0] // 2, :] = 0
        weight[:, :, :, self.kernel_size[1] // 2] = 0
        return torch.tensor(weight.copy(), dtype=torch.float32)
