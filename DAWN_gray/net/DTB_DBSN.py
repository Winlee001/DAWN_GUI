...
from net.restormer_arch import DTB
...
class DTB_DBSN(nn.Module):
    def __init__(self, in_ch, out_ch, mid_ch, 
                ...
                activate_fun,head=True):
        super(DTB_DBSN,self).__init__()
        #
        if activate_fun == 'Relu':
            # self.relu = nn.ReLU(inplace=True)
            self.relu = partial(nn.ReLU, inplace=True)
        ...
        if blindspot_conv_type == 'MulMask':
            self.br1 = DBSN_branch(mid_ch,'MulMask', blindspot_conv_bias, br1_blindspot_conv_ks, br1_block_num, activate_fun)
            self.br2 = DTB(dim=mid_ch,num_blocks=[br2_block_num], stride=3)
        else:
            self.br1 = DBSN_branch(mid_ch, blindspot_conv_type, blindspot_conv_bias, br1_blindspot_conv_ks, br1_block_num, activate_fun)
            self.br2_blind_conv = BlindSpotConv(mid_ch, mid_ch, br2_blindspot_conv_ks, stride=1, dilation=1, bias=blindspot_conv_bias, conv_type=blindspot_conv_type)
            self.br2 = DTB(dim=mid_ch,num_blocks=[br2_block_num], stride=3)
            init_weights(self.br2_blind_conv)
    ...
    def forward(self, x):
        if self.head:
            x = self.dbsn_head(x)
        x1 = self.br1(x)     
        x2 = self.br2(self.br2_blind_conv(x1)) 
