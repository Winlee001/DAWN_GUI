...
'''
extend() 是 Python 中列表（list）的一个内置方法，用于将一个可迭代对象（如另一个列表、元组、字符串等）的所有元素追加到当前列表中。
与 append() 方法不同，extend() 不会将整个对象作为一个元素添加，而是将对象中的每个元素单独添加到列表中。
'''
...
class imlib():
    def __init__(self, mode='RGB', fmt='CHW', lib='cv2', h5file=None):
        ...
        #TODO 重要修改，新增mode:L_16
        assert mode.upper() in ('RGB', 'L','L_16')
        self.mode = mode.upper()

        assert fmt.upper() in ('HWC', 'CHW', 'NHWC', 'NCHW')
        self.fmt = 'CHW' if fmt.upper() in ('CHW', 'NCHW') else 'HWC'

        assert lib.lower() in ('cv2', 'pillow', 'h5','tiff')
        self.lib = lib.lower()
    ...
    # TODO 修改如下
    def _imread_cv2_L_16(self, path):
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)
        return img
    def _imread_tiff_L_16(self, path):
        try:
            img = tiff.imread(path)
            if img.dtype == np.uint16 and img.shape[1] != 1:# 第三维为帧数，保证读取帧数不为1
                return img
            else:
                raise ValueError('check Image type')
        except Exception as e:
            print(f"Error reading TIFF file {path}: {e}")
    ...
    #TODO imwrite的函数
    def _imwrite_cv2_L_16(self,image,path):
        cv2.imwrite(path,image)

    def _imwrite_tiff_L_16(self,image,path):
        tiff.imwrite(path, image,dtype=np.uint16)
    ...
    #TODO
    def _trans_image_L_16_CHW(self, image):
        #如果是HW灰度图，即单帧灰度图
        if image.ndim == 2:
            return np.ascontiguousarray(np.expand_dims(image, 0))
        #如果是多帧灰度图，即HWC
    ...
    def read(self, paths):
        ...
        #事实上源代码gray中只有这步会运行
        # print(paths,tiff.imread(paths).shape)
